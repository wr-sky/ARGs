...
class MS_dataset:
    ...
    def load_dataset(self, path):
        ...
        for i in range(len(self.scaled_data)):
            ...
            self.dict = {"emb_1": self.vector_1.view(-1, 512), "emb_2": self.vector_2.view(-1, 1280),"label": self.label}
...
class ProtAlbertClassifier(pl.LightningModule):
    ...
    def __build_model(self) -> None:
        ...
        self.classification_head_2 = nn.Sequential(
            ...
            LSTMWithMultiheadAttention(input_size=1280, hidden_size=512, num_layers=5, num_heads=4),
            # nn.LSTM(input_size=5120, hidden_size=5120, num_layers=5, batch_first=True, dropout=0.256),
            LSTMFlatten(),
    ...
    def loss(self, predictions_1: dict, predictions_2: dict, targets:dict):
        ...
        return self._loss_1(predictions_1["logits"], targets)*0.6+self._loss_2(predictions_2["logits"], targets)*0.4
    ...
    def validation_step(self, batch: tuple, batch_nb: int, *args, **kwargs) -> dict:
        ...
        for i in range(len(labels_hat_1)):
            if labels_hat_1[i] == labels_hat_2[i]:
                ...
                max_prob_1, max_index_1 = torch.max(y_hat_1[i], dim=0)
                max_prob_2, max_index_2 = torch.max(y_hat_2[i], dim=0)
                if max_prob_1 >= max_prob_2:
                    label_hat = max_index_1.item()
                    label_hat = torch.tensor(label_hat).to("cuda:0")
                    labels_hat.append(label_hat)
                else:
                    label_hat = max_index_2.item()
                    label_hat = torch.tensor(label_hat).to("cuda:0")
                    labels_hat.append(label_hat)

        labels_hat = torch.stack(labels_hat)
        val_acc = self.metric_acc(labels_hat, targets)
        loss_val = self.loss(model_out_1,model_out_2, targets)
    ...
    def test_epoch_end(self, outputs: list) -> dict:
        ...
        tqdm_dict = {"test_loss": test_loss_mean, "test_acc": test_acc_mean, "f1_score": f1_mean,
                     "precision": precision_mean, "recall": recall_mean}
