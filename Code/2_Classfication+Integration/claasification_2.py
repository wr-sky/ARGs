...
class MS_dataset:
    ...
    def load_dataset(self, path):
        ...
        for i in range(len(self.strain_vector)):
            ...
            self.strain_len = len(self.strain_vectors)
            self.padding_len = 30720 - self.strain_len
            self.zero_tensor = torch.zeros(self.padding_len)
            self.strain_vectors = torch.cat((self.strain_vectors, self.zero_tensor))
            ...
            self.strain_dict = {"emb": scaled_emb.view(-1, 256), "label": self.strain_label}
...
class LuongAttention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        hidden = hidden.expand(encoder_outputs.size(1),-1, -1)
        # print("encoder_outputs", hidden.expand(encoder_outputs.size(0),-1, -1).shape)

        # 计算注意力权重
        attention_scores = torch.matmul(self.attention(encoder_outputs), hidden.transpose(0, 1).transpose(1, 2))
        attention_weights = torch.softmax(attention_scores, dim=1)

        # 计算上下文向量
        context_vector = torch.bmm(encoder_outputs.transpose(1, 2),attention_weights).squeeze(1)

        return context_vector, attention_weights


class LSTMWithLuongAttention(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        super(LSTMWithLuongAttention, self).__init__()
...
class ProtAlbertClassifier(pl.LightningModule):
    ...
    def __build_model(self) -> None:
        ...
        # self.encoder_features = 29500000

        self.hidden_size_1 = 61440
        self.hidden_size_2 = 128
        self.hidden_size_3 = 256
        # self.hidden_size_4 = 512
        # self.hidden_size_5 = 1024

        self.classification_head = nn.Sequential(
            # nn.Conv1d(in_channels=60, out_channels=150, kernel_size=3, stride=1, padding=0),
            # nn.GELU(),
            # nn.Conv1d(in_channels=150, out_channels=1, kernel_size=3, stride=1, padding=0),
            # nn.GELU(),
            # nn.Dropout(p=0.3),
            LSTMWithLuongAttention(input_size=256, hidden_size=512, num_layers=4),
            LSTMFlatten(),
    ...
    def loss(self, predictions: dict, targets:dict):
        # a = predictions["logits"]
        # b = targets

        return self._loss(predictions["logits"], targets)

    # def add_gaussian_noise(self,input_data, mean=0, std=0.1):
    #     self.noise = torch.randn_like(input_data) * std + mean
    #     self.noisy_data = input_data + self.noise
    ...
    def prepare_sample(self, sample):
        ...
        # inputs = self.add_gaussian_noise(input,mean=0,std=0.3)
    ...
    def add_model_specific_args(
        ...
        parser.add_argument(
            "--train_csv",
            default="/data/newdisk/mrj_data/resistance/data/train_data.pkl",
            type=str,
            help="Path to the file containing the train data.",
        )
        parser.add_argument(
            "--dev_csv",
            default="/data/newdisk/mrj_data/resistance/data/val_data.pkl",
            type=str,
            help="Path to the file containing the dev data.",
        )
        parser.add_argument(
            "--test_csv",
            default="/data/newdisk/mrj_data/resistance/data/test_data.pkl",
            type=str,
            help="Path to the file containing the dev data.",
        )
...
# Batching
parser.add_argument("--batch_size", default=100, type=int, help="Batch size to be used.")
parser.add_argument(
