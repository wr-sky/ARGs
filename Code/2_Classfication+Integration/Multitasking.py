...
class MS_dataset:
    ...
    def load_dataset(self, path):
        ...
        for i in range(len(self.scaled_data)):
            ...
            self.dict = {"emb_1": self.vector_1.view(-1, 1280), "emb_2": self.vector_2.view(-1, 1280),"label": self.label}
...
class LSTM_layer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        ...
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=0.256)
...
class LSTMWithMultiheadAttention(nn.Module):
    def __init__(self, lstm_shared, num_heads):
        super(LSTMWithMultiheadAttention, self).__init__()

        self.encoder = lstm_shared
        self.attention = MultiheadAttention(lstm_shared.hidden_size, num_heads)
...
class ProtAlbertClassifier(pl.LightningModule):
    ...
    def __build_model(self) -> None:
        ...
        self.hidden_size_1_1 = 12288
        self.hidden_size_1_2 = 512
        self.hidden_size_2 = 1024
        self.hidden_size_3 = 2048

        lstm_shared = LSTM_layer(input_size=1280, hidden_size=512, num_layers=4)
        ...
        self.classification_head_2 = nn.Sequential(
            ...
            LSTMWithMultiheadAttention(lstm_shared, num_heads=4),
            # LSTM_layer(input_size=2560, hidden_size=512, num_layers=8),
            LSTMFlatten(),
    ...
    def loss(self, predictions_1: dict, predictions_2: dict, targets:dict):
        ...
        return self._loss_1(predictions_1["logits"], targets)*0.65+self._loss_2(predictions_2["logits"], targets)*0.35
    ...
    def validation_step(self, batch: tuple, batch_nb: int, *args, **kwargs) -> dict:
        # label_correct_val_counts = {label: 0 for label in range(16)}
        ...
        # print("11:",y_hat_1[0])
        # print("11shape:",y_hat_1.shape)
        labels_hat_1 = torch.argmax(y_hat_1, dim=1)

        # print("111:",labels_hat_1.type())
        # val_acc_1 = self.metric_acc(labels_hat_1, y_1)

        model_out_2 = self.forward_2(inputs_2)
        y_2 = targets
        y_hat_2 = model_out_2["logits"]
        labels_hat_2 = torch.argmax(y_hat_2, dim=1)
        # print("222:",labels_hat_2[0])
        # val_acc_2 = self.metric_acc(labels_hat_2, y_2)
        labels_hat = []
        # a = 0
        # b = 0
        for i in range(len(labels_hat_1)):
            if labels_hat_1[i] == labels_hat_2[i]:
                ...
                # print("lll1:", label_hat)
                labels_hat.append(label_hat)
                # if label_hat == targets[i]:
                    # label_correct_val_counts[int(targets[i])] += 1
                # print("hhh1:",labels_hat)
                # a = a +1
            else:
                y_hat = y_hat_1[i]*0.35 + y_hat_2[i]*0.65
                label_hat = torch.argmax(y_hat)
                labels_hat.append(label_hat)
                # if labels_hat == targets[i]:
                    # label_correct_val_counts[int(targets[i])] += 1
                # print("hhh2:",labels_hat)
                # b = b+1

        # print("a:", a)
        # print("b:", b)
        # print("333:", labels_hat)
        labels_hat = torch.stack(labels_hat)
        # print("444:", labels_hat)
        val_acc = self.metric_acc(labels_hat, y_1)
        loss_val = self.loss(model_out_1,model_out_2, targets)

        # print("label_correct_val_counts:", label_correct_val_counts)

        output = OrderedDict(
            {
                "val_loss": loss_val,
                "val_acc": val_acc,
                # "label_correct_val_counts": label_correct_val_counts,
            }
        )
    ...
    def validation_epoch_end(self, outputs: list) -> dict:
        # label_correct_val_counts_end = {label: 0 for label in range(16)}

        val_loss_mean = torch.stack([x["val_loss"] for x in outputs]).mean()
        val_acc_mean = torch.stack([x["val_acc"] for x in outputs]).mean()

        # for output in outputs:
        #     label_counts = output["label_correct_val_counts"]
        #     for label in label_correct_val_counts_end:
        #         label_correct_val_counts_end[label] += label_counts[label]
        ...
        result = {
            ...
            # "label_correct_val_counts_end": label_correct_val_counts_end,
        }
        return result
    ...
    def test_step(self, batch: tuple, batch_nb: int, *args, **kwargs) -> dict:
        ...
        labels_hat = torch.stack(labels_hat)
        test_acc = self.metric_acc(labels_hat, y_1)
        loss_test = self.loss(model_out_1,model_out_2, targets)

        f1 = f1_score(y_1, labels_hat, average='weighted')
        precision = precision_score(y_1, labels_hat, average='weighted')
        recall = recall_score(y_1, labels_hat, average='weighted')

        # print("label_correct_test_counts:", label_correct_test_counts)
